...
class ArticleController extends Controller
{
    ...
    public function actionIndex($cat = '')
    {
        ...
        if(!empty($cat_id)){
            $where = ['type' => Article::ARTICLE, 'status' => Article::ARTICLE_PUBLISHED, 'cid'=>$cat_id];
        }else{
            $where = ['type' => Article::ARTICLE, 'status' => Article::ARTICLE_PUBLISHED];
        }

        $query = Article::find()->orderby('sort ASC,id DESC')->where($where);
    ...
    public function actionRss()
    {
        ...
        $models = Article::find()->limit(20)->where(['status'=>Article::ARTICLE_PUBLISHED, 'type'=>Article::ARTICLE])->orderBy('id desc')->all(); 
