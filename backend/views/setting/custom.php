        ...
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content">
                ...
                foreach ($settings as $index => $setting) {
                    ...
                    $template = "{label}\n<div class=\"col-sm-8\">{input}\n{error}</div>\n{hint}<div class='col-sm-2'><span class='tips'><i class='fa fa-info-circle'></i> {$setting->tips}  <a href='{$editUrl}' class='btn_edit' title='编辑' data-pjax=''><i style='float: right;margin-right: 10px;' class='fa fa-pencil'></i></a> </span></div>";
                    if ($setting->input_type == Constants::INPUT_UEDITOR) {
                        ...
                        if ($setting->input_type == Constants::INPUT_INPUT) {
                            echo $form->field($setting, "[$index]value", ['template' => $template])
                                ->label(Yii::t('app',ucwords($setting->name)))
                                ->textInput();
                        } else {
                            echo $form->field($setting, "[$index]value", ['template' => $template])
                                ->label(Yii::t('app',ucwords($setting->name)))
